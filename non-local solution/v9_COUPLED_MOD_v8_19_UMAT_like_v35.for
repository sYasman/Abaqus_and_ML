...
! C
	  ...
	  dg_dEps_V = 0.d0
	  dgNeg_dEps_V = 0.d0
	  delta_eps_eq_neg = 0.d0
	  delta_eps_eq_pos = 0.d0
	  dEpsPl_neg_dEps_V = 0.d0
	  dStress_dEpsBar_neg_V = 0.d0
...
!C**
     ...
     3  sigma_c, sigma_t, phi_tr, d_m, d_m_neg, eps_eq_n_neg)    
	   ! phi_tr = -1.d0 !test icin. plasticity yi kapadim.
C
...
! C **  
     ...
     6  d_m_neg, KINC, G_TENS, G_COMP, GTENS_A, GTENS_B,
     7  delta_eps_eq_neg, delta_eps_eq_pos, DTIME, dg_dEps_V, dgNeg_dEps_V)  
...
C    
     ...
     9  pl_stress, d_mOLD, d_mOLD_neg, kappa_old, kappa_old_neg, total_strain,
     1  G_TENS, G_COMP) ! Update tangent
...
C
     ...
     3 eps_eq_new_neg, d_m_neg, G_TENS, G_COMP)
C**
...
C INPUT        
     ...
     4  c_neg, eps_eq_new_neg, d_m_neg, G_TENS, G_COMP
...
C
        ...
        dSc_dGamma = 0.d0
C
        dEpsEq_pos_dEpsEq = G_TENS * c_pos
        dEpsEq_neg_dEpsEq = G_COMP * c_neg
        do i=1,3
          do j=1,3
...
C
     ...
     7  KINC, G_TENS, G_COMP, GTENS_A, GTENS_B,
     8  delta_eps_eq_neg, delta_eps_eq_pos, DTIME, dg_dEps_V,
     9  dgNeg_dEps_V)
        implicit none
! C**
...
! CCCC OUTPUT
     ...
     3   pres_new, eps_eq_new_neg, c_pos, c_neg, G_TENS, G_COMP,
     4   delta_eps_eq_neg, delta_eps_eq_pos, dg_dEps_V(6), dgNeg_dEps_V(6)	 
! C     
        integer, intent(in) :: KINC
! C        
...
! CCCC Modifications in delta_eps_eq_neg and delta_eps_eq_neg  
     ...
     3   Cel_4D(3,3,3,3), dg_dEps(3,3)	 
        ...
        real*8 gamma_u, gamma_l, f_gamma_u, f_gamma_l		
! CCCC CODE
        diff = 1.0d0
        TOL = 1e-6
		TOL = 1e-3
...
!      real*8 gamma, gamma_old, f_gamma_u, f_gamma_l
	     gamma_u = 0.d0
		 gamma_l = 1.d0
        do while(diff.GT.TOL.AND.iter_num.LT.iter_max) ! local N-R
!! Compute phi values
! Compute f_gamma_u
         stress_dev_new = stress_dev_tr / (1.d0+6.d0*mu*gamma_u)		  
         pres_new = pres_tr / (1.d0+2.d0*gamma_u*alpha*kappa)
         stress_new = stress_dev_new + I_1*pres_new			
! C	
		 CALL getG_MULTIPLERS(stress_new, GTENS_A, GTENS_B, G_TENS, G_COMP)		
! C		
         delta_eps_p= gamma_u *
...
! MOD: G_TENS and G_COMP eklendi
         eps_eq_new = eps_eq_n + G_TENS*delta_eps_eq_pos  ! positive driving force
		 eps_eq_new_neg = eps_eq_n_neg + G_COMP*delta_eps_eq_neg ! negative diriving force		  
! C
...
! C** Compute Phi
         ...
         CALL getPhi(nr_stres_dev_new,pres_new,sigma_c,sigma_t,f_gamma_u)
		 	 
! Compute f_gamma_old
         stress_dev_new = stress_dev_tr / (1.d0+6.d0*mu*gamma_l)		  
         pres_new = pres_tr / (1.d0+2.d0*gamma_l*alpha*kappa)
         stress_new = stress_dev_new + I_1*pres_new			
! C	
		 CALL getG_MULTIPLERS(stress_new, GTENS_A, GTENS_B, G_TENS, G_COMP)		
! C		
...
! C** Compute Phi
         CALL normOf(stress_dev_new, nr_stres_dev_new)
         CALL getPhi(nr_stres_dev_new,pres_new,sigma_c,sigma_t,f_gamma_l)
 
! Compute gamma_cur
! 5e-2 is best so far
         gamma_cur = 0.5d0 * (gamma_u + gamma_l)
! Compute diff
         stress_dev_new = stress_dev_tr / (1.d0+6.d0*mu*gamma_cur)		  
         pres_new = pres_tr / (1.d0+2.d0*gamma_cur*alpha*kappa)
         stress_new = stress_dev_new + I_1*pres_new			
...
! C
        ...
        enddo !end of local bi-section   
! compute phi_pr with new gamma_cur        
		CALL getPhiPr(nr_stres_dev_tr, mu, gamma_cur, pres_tr,
     1 alpha, kappa, kk, delta_eps_p, delta_eps_eq, stress_dev_tr,
     2 I_1, a_t, b_t, c_t, d_t, a_c, b_c, c_c, d_c, eps_eq_new,
     3 sigma_c, sigma_t, iter_num, phi_pr, d_m, c_pos, c_neg, 
     4 eps_eq_new_neg, d_m_neg, G_TENS, G_COMP)!COMPUTES D(PHI)/D(GAMMA)		
! C**
...
! dS1pr_dStress(3,3), dS3pr_dStress(3,3), dS2pr_dStress(3,3),	
       CALL SPRIND(stress_new, sig_pr, sig_cos, 1, 3, 3)
	   s1_pr = sig_pr(1)
	   s2_pr = sig_pr(2)
	   s3_pr = sig_pr(3)
	   
	   s1_pr = MAXVAL(sig_pr)
	   s1_ind = MAXLOC(sig_pr, 1)
	   
	   s3_pr = MINVAL(sig_pr)
	   s3_ind = MINLOC(sig_pr, 1)	   
       do i=1,3
	    if((i.NE.s1_ind).AND.(i.NE.s3_ind))then
		 s2_ind = i
		 s2_pr = sig_pr(s2_ind)
		endif
       enddo
	   ...
	   n2 = sig_cos(s2_ind, :)
	   n3 = sig_cos(s3_ind, :)
	   dg_dS1 = -(GTENS_A*GTENS_B*(((s1_pr+s2_pr+s3_pr)*1.0D0/sqrt(s1_pr**2+
     &s2_pr**2+s3_pr**2))/3.0D0)**GTENS_B*(s1_pr*s2_pr+s1_pr*s3_pr-s2_pr*
     &*2-s3_pr**2))/((s1_pr+s2_pr+s3_pr)*(s1_pr**2+s2_pr**2+s3_pr**2))
	   dg_dS2 = -(GTENS_A*GTENS_B*(((s1_pr+s2_pr+s3_pr)*1.0D0/sqrt(s1_pr**2+
     &s2_pr**2+s3_pr**2))/3.0D0)**GTENS_B*(s1_pr*s2_pr+s2_pr*s3_pr-s1_pr*
     &*2-s3_pr**2))/((s1_pr+s2_pr+s3_pr)*(s1_pr**2+s2_pr**2+s3_pr**2))
	   dg_dS3 = -(GTENS_A*GTENS_B*(((s1_pr+s2_pr+s3_pr)*1.0D0/sqrt(s1_pr**2+
     &s2_pr**2+s3_pr**2))/3.0D0)**GTENS_B*(s1_pr*s3_pr+s2_pr*s3_pr-s1_pr*
     &*2-s2_pr**2))/((s1_pr+s2_pr+s3_pr)*(s1_pr**2+s2_pr**2+s3_pr**2)) 
	   do i=1,3
	    do j=1,3
		 dS1pr_dStress(i,j) = n1(i)*n1(j)
		 dS2pr_dStress(i,j) = n2(i)*n2(j)
		 dS3pr_dStress(i,j) = n3(i)*n3(j)
        enddo
       enddo
	   ...
	   do i=1,3
	    do j=1,3
		 do k=1,3
		  do l=1,3
		   dg_dEps(k,l) = dg_dEps(k,l) +
     1 (dg_dS1 * dS1pr_dStress(i,j) + dg_dS2 * dS2pr_dStress(i,j) +
     2 dg_dS3 * dS3pr_dStress(i,j)) * Cel_4D(i,j,k,l)
		  enddo
         enddo
        enddo
       enddo	
       dg_dEps_V(1) = dg_dEps(1,1)
       dg_dEps_V(2) = dg_dEps(2,2)
       dg_dEps_V(3) = dg_dEps(3,3)
       dg_dEps_V(4) = dg_dEps(1,2)
       dg_dEps_V(5) = dg_dEps(1,3)
       dg_dEps_V(6) = dg_dEps(2,3)	   
	   
! COmputation of d_Gneg_d_eps
       s3_pr = s3_pr + 1e-3 ! to provide numerical stability and reduce 1/1e-6 like errors
	   
	   dg_dS1 = 1.0D0/(abs(s3_pr)*3.0D0)
	   dg_dS2 = 1.0D0/(abs(s3_pr)*3.0D0)
	   dg_dS3 = 1.0D0/abs(s3_pr)**2*(-abs(s3_pr)+s1_pr*((s3_pr/abs(s3_pr)))+
     1  s2_pr*((s3_pr/abs(s3_pr)))+s3_pr*((s3_pr/abs(s3_pr))))*(-1.0D0/3.0D0)
     ...
     2 dg_dS3 * dS3pr_dStress(i,j)) * Cel_4D(i,j,k,l)
        ...
        enddo
       enddo
       dgNeg_dEps_V(1) = dgNeg_dEps(1,1)
       dgNeg_dEps_V(2) = dgNeg_dEps(2,2)
       dgNeg_dEps_V(3) = dgNeg_dEps(3,3)
       dgNeg_dEps_V(4) = dgNeg_dEps(1,2)
       dgNeg_dEps_V(5) = dgNeg_dEps(1,3)
       dgNeg_dEps_V(6) = dgNeg_dEps(2,3)	
...
! C 9. dSigmaT_dEps (3,3)
        dEpsEq_pos_dEpsEq = c_pos * G_TENS! since dEps_EQ_pl_pos = dEps_EQ_pl * c_pos
        dEpsEq_neg_dEpsEq = c_neg * G_COMP! since dEps_EQ_pl_neg = dEps_EQ_pl * c_neg
! C      
        do i=1,3
         do j=1,3
          do k=1,3
...
! C
		if(ABS(K2).LT.1e-6)then ! prevent 1/0 errors.
		 K2 = 1e-6
		endif
C		
        do i=1,3
...
! ! With below Von-Mises is obtained, sc = st
        ! a_c=86.29d0
        ! b_c=3.775d0
        ! c_c=-61.29d0
        ! d_c=-241.d0  		
...
! C        
      ...
      subroutine getG_MULTIPLERS(stress_new, GTENS_A, GTENS_B, G_TENS, G_COMP)
        implicit none	  
! INPUT
        real*8, intent(in) :: stress_new(6), GTENS_A, GTENS_B
! OUTPUT
        real*8, intent(out) :: G_TENS, G_COMP
! LOCAL
        real*8 PS(3), S1, S3, S_H, norm_stress, S_M	
! real*8 S1, S2, S3, S_H, GTENS_A, GTENS_B, norm_stress, G_TENS		
		  CALL SPRINC(stress_new,PS,1,3,3)
		  S1 = MAXVAL(PS)
		  S3 = MINVAL(PS)
		  S_H = (1.d0/3.d0) * (PS(1) + PS(2) + PS(3))
		  S_M = PS(1) + PS(2) + PS(3)

		  norm_stress = SQRT(PS(1)**2.d0 + PS(2)**2.d0 + PS(3)**2.d0)
		  if(S_H.GT.0.d0)then 
		    G_TENS = 1.d0 + GTENS_A * (S_H/ norm_stress)**GTENS_B
			! G_TENS = 1.d0
! for negative part
		  elseif(S_H.LT.0.d0)then	
		    G_COMP = 1.d0 + S_H/ABS(S3)	
			! G_COMP = 1.d0
		  endif		
	  endsubroutine getG_MULTIPLERS
	  
	  
      subroutine updateDamageStrain(pl_stress, d_mOLD, d_mOLD_neg,
     1    kappa_str, kappa_old, kappa_str_neg, kappa_old_neg, 
     2    aa, bb, d_m_NEW, d_m_neg_NEW)
        implicit none	  
! INPUT
        real*8, intent(in) :: pl_stress(6), d_mOLD, d_mOLD_neg,
     1    kappa_str, kappa_old, kappa_str_neg, kappa_old_neg, 
     2    aa, bb
! OUTPUT
        real*8, intent(out) :: d_m_NEW, d_m_neg_NEW
! LOCAL
        real*8 d_m, d_m_neg, PS(3), S_H
		
! SPRINC(VECTOR_TO_PRINCPIALS, PS_VECTOR, TYPE(1:=STRESS, 2:=STRAIN), NDI, NSHR)		
        CALL SPRINC(pl_stress,PS,2,3,3) ! For the case when STRAIN is used for tension and compression distinction		
        S_H = (PS(1) + PS(2) + PS(3)) ! volumetric strain
		
! C** LOGIC: If volumetric strain is Positive then it is tension case
! C** Otherwise it is compression case		
		
        if(S_H.GT.0.d0.AND.kappa_str.GT.kappa_old) then
          CALL exp_damage(kappa_str, d_m) ! compute new TENSION damage
        endif  
        d_m_NEW = MAX(d_m, d_mOLD)
!
        if(S_H.LT.0.d0.AND.kappa_str_neg.GT.kappa_old_neg) then
          CALL exp_damage_comp(kappa_str_neg, d_m_neg) ! compute new COMPRESSION damage
        endif     
        d_m_neg_NEW = MAX(d_m_neg, d_mOLD_neg)
      endsubroutine updateDamageStrain  
! C**
! C**
      subroutine updateDamageStress(pl_stress, d_mOLD, d_mOLD_neg,
     1    kappa_str, kappa_old, kappa_str_neg, kappa_old_neg, 
     2    aa, bb, d_m_NEW, d_m_neg_NEW)
        implicit none	  
! INPUT
...
! SPRINC(VECTOR_TO_PRINCPIALS, PS_VECTOR, TYPE(1:=STRESS, 2:=STRAIN), NDI, NSHR)		
        CALL SPRINC(pl_stress,PS,1,3,3) ! For the case when STRESS is used for tension and compression distinction		
        S_H = (1.d0/3.d0) * (PS(1) + PS(2) + PS(3))
		
! C** LOGIC: If pressure is Positive then it is tension case
! C** Otherwise it is compression case				
...
!
        ...
        d_m_neg_NEW = MAX(d_m_neg, d_mOLD_neg)
      endsubroutine updateDamageStress  
! C**
! C**
      subroutine get_driving_forces(pl_stress, d_mOLD, d_mOLD_neg,
     1    kappa_str, kappa_old, kappa_str_neg, kappa_old_neg, 
     2    aa, bb, nr_stres_dev_tr, mu, pres_tr, alpha,
     1    kappa, kk, stress_dev_tr, I_1, a_t, b_t,
     2    c_t, d_t, a_c, b_c, c_c, d_c, eps_eq_n, eps_pl_n2D,
     3    phi_tr, fact_neg_stress, eps_eq_n_neg, GTENS_A, GTENS_B,
     4    KINC, PNEWDT, eps_eq_drive, eps_eq_drive_neg,
     5    d_m_NEW, d_m_neg_NEW)
        implicit none	  
! INPUT
        real*8, intent(in) :: pl_stress(6), d_mOLD, d_mOLD_neg,
     1    kappa_str, kappa_old, kappa_str_neg, kappa_old_neg, 
     ...
     2    c_t, d_t, a_c, b_c, c_c, d_c, eps_eq_n, eps_pl_n2D(3,3),
     3    phi_tr, fact_neg_stress, eps_eq_n_neg, GTENS_A, GTENS_B,
     4    d_m_NEW, d_m_neg_NEW
! OUTPUT
        real*8, intent(out) :: eps_eq_drive, eps_eq_drive_neg
C		
        integer, intent(in) :: KINC
C        
...
! LOCAL
     ...
     2   eps_pl_new2D(3,3), stress_new(3,3), stress_dev_new(3,3),
     3   pres_new, eps_eq_new_neg, c_pos, c_neg, G_TENS, G_COMP 
! C** CALCULATION OF THE LOCAL DRIVING FORCES OF NON-LOCAL SOLUTION VARIABLES
! C** eps_eq_drive: LOCAL DIRING FORCE OF POSITIVE EQUIVALENT PLASTIC STRAIN PDE (PDE-2)
! C** eps_eq_drive_neg: LOCAL NEGATIVE FORCE OF POSITIVE EQUIVALENT PLASTIC STRAIN DPE (PDE-3)
! 	LOGIC
! with new damage values, compute required terms for Internal Force Columns
! However, mechanical part of the solution must consist of only step n's damage terms
! Then, in this part only eps_eq_drive and eps_eq_drive_neg will be used.
! Other quantities will be again computed by using NOT-UPDATED damage terms
C        ! CALL updateDamageStress(pl_stress, d_mOLD, d_mOLD_neg,
C     ! 1    kappa_str, kappa_old, kappa_str_neg, kappa_old_neg, 
C     ! 2    aa, bb, d_m_NEW, d_m_neg_NEW)
      
        CALL plasticCorrector(nr_stres_dev_tr, mu, pres_tr, alpha,
     1   kappa, kk, stress_dev_tr, I_1, a_t, b_t, c_t, d_t,a_c,b_c,c_c,
     2   d_c, eps_eq_n, eps_pl_n2D, phi_tr, delta_eps_p, delta_eps_eq,
     3   eps_eq_new, sigma_c, sigma_t, phi_pr, gamma_cur, eps_pl_new2D,
     4   stress_new, stress_dev_new, pres_new, d_m_NEW, PNEWDT,
     5   fact_neg_stress, eps_eq_n_neg, eps_eq_new_neg, c_pos, c_neg, 
     6   d_m_neg_NEW, KINC, G_TENS, G_COMP, GTENS_A, GTENS_B)
! C   
! C* eps_eq_new is NEW LOCAL equivalent plastic strain
! C* It is also, driving force like quantity for second PDE (non-local part)
! C* Problem: In the derivations of the local driving forces D_N terms are used. 
! C* However in those calculations D_NEW quantities must be used.
! C* CRUDE solution, at first obtain driving local forces with new damage
! C* Then, using old damage quantities do everything again. (Compute pl_stress)
          eps_eq_drive = eps_eq_new ! new local quantitty for 2nd PDE (positive equivalent plastic strain)                
          eps_eq_drive_neg = eps_eq_new_neg ! new local quantitty for 3nd PDE (negative equivalent plastic strain)  
! C** END OF THE CALCULATION OF THE LOCAL DRIVING FORCES OF NON-LOCAL SOLUTION VARIABLES	
      endsubroutine get_driving_forces  
	  
      subroutine updateDTIME(DTIME, DMAX)
        implicit none	  
! INPUT
		real*8, intent(in) :: DMAX
        real*8, intent(inout) :: DTIME  
! Local quantities		
		real*8 DT_1, DT_2, DT_3, DT_4
! Those quantities must be updated for each model		
		DT_1 = 5e-4 ! bunu azalt.
		DT_2 = 5e-5
		DT_3 = 1e-5	
		DT_4 = 5e-6

		if((DMAX.GT.0.d0).AND.(DMAX.LT.0.40d0))then
		 DTIME = DT_1
		elseif((DMAX.GE.0.40d0).AND.(DMAX.LT.0.85d0))then
		 DTIME = DT_2	
		elseif((DMAX.GE.0.85d0).AND.(DMAX.LT.0.90d0))then
		 DTIME = DT_3
		elseif(DMAX.GE.0.90d0)then
         DTIME = DT_4	
		endif		
	  endsubroutine updateDTIME 	  